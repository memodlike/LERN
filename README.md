...
CSV и TSV: `text` / `quote` / `body` / `content`, необязательные `author`, `source`, `tags`, `section` или `category`. CSV определяет запятую, точку с запятой или tab по форме записей, показывает спорный результат и позволяет явно выбрать разделитель и наличие заголовка. Строки разной ширины остаются импортируемыми, но показываются в предупреждениях. TXT: строки либо абзацы, и выбранная раскладка видна в предпросмотре. Markdown: абзацы, списки, цитаты, заголовки `#`–`######` с пробелом и необязательные `author`, `source`, `tags` во front matter; fenced code с ``` или `~~~` остаётся текстом и не создаёт разделов. JSON: массив строк либо объектов с теми же полями; имена полей регистронезависимы, а конфликтующие варианты одного поля отклоняются. JSONL: объект или строка на каждой непустой строке. Примеры находятся в `Fixtures/import`.
...
Импортированные библиотеки можно переименовать, приостановить, возобновить, просмотреть и удалить из меню библиотеки. Пауза исключает библиотеку из выбранного источника, не расширяя его до всех записей. Удаление удаляет только её связи и дочерние разделы: общие записи сохраняются, а избранные записи без других связей переходят в «My Content». Замена меняет связи выбранной библиотеки; сами записи, их избранное и связи с другими коллекциями сохраняются. Отмена откатывает текущий файл; уже завершённые файлы остаются в библиотеке. Предпросмотр показывает повторы внутри файла, число разделов, ограничение текста уведомлений и первые структурированные предупреждения. Записи совпадают по нормализованному тексту, автору и источнику: регистр, пробелы, теги и раздел не создают новую запись. Для объединённого импорта в деталях библиотеки сохраняется список всех файлов-источников.

Книги CSV: `title,author,note,url`. JSON: массив объектов с этими строковыми полями. Перед сохранением показывается предпросмотр с корректными, повторными и пропущенными строками. Повторы определяются по нормализованным названию, автору и URL; избранные существующие материалы не меняются. Лимит файла — 10 MB включительно, библиотеки — 10 000 материалов; поля названия, автора и URL — до 2 000 символов, заметка — до 20 000. Примеры находятся в `Fixtures/resources`.
